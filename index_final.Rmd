...
p < 0.001), (X²(1) = 3921.2, df = 1, p < 0.001) including when compared to 
the random intercept (L(4) = 723.05, p < 0.0001) which was also shown by the lower AICc scores.
The relationship between the session and test score showed significant
variance in intercepts across sessions, SD = 3.84 (95% Cl 2.87, 5.14). 
In addition, We found that the interclass correlation between 
participants is very high (0.94).
...
We can see that the means of all coefficients are above 0 which means they are all relevant for the performance of the model, with a and sigma_subject having a higher mean and standard deviations of 0.88 and 0.61 while b and sigma have lower means around 1 but also smaller standard deviations of 0.02 and 0.04 with their confidence intervals not falling bellow 0.
