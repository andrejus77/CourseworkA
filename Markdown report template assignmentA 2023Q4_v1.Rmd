...
Researchers found that Nicotine caused weight loss in [rats](https://www.sciencedirect.com/science/article/abs/pii/0014299976902016) and then later in [humans](https://onlinelibrary.wiley.com/doi/abs/10.1111/j.1360-0443.1991.tb01810.x). Morean and Wedel [noticed](https://www.sciencedirect.com/science/article/pii/S0306460316303781?casa_token=Fuxiv7q95AwAAAAA:obMaLgmaIEyb-eH0Bb-_x-rL0bLz-ox_8eXxWqSw6gScAA7dnlXkUMgyfJO3v2gatjjAPio8Rx0) that 13.5% of adults in USA vape to lose weight.

## The theory underlying the research  
(Max 200 words) Preferable based on theories reported in literature

There are a few theories for how Nicotine reduces weight through [Leptin](https://www.sciencedirect.com/science/article/pii/S0195666321001677?casa_token=-HTtrT14rR0AAAAA:KI3zc_yYySluosZSz2MJrDsdsF-yTb8MOIqKaUtYKA6Ug_QOOo1iRGys0zYQweEEf54SRgoESjY), [fight-or-flight receptors](https://www.npr.org/2011/06/09/137085989/the-skinny-on-smoking-why-nicotine-curbs-appetite) but also indications that weight should increase through [Ghrelin](https://www.sciencedirect.com/science/article/pii/S0195666321001677?casa_token=-HTtrT14rR0AAAAA:KI3zc_yYySluosZSz2MJrDsdsF-yTb8MOIqKaUtYKA6Ug_QOOo1iRGys0zYQweEEf54SRgoESjY). However, all of these sources note that there is empirical evidence showing strong correlation between Nicotine consumption and weight loss. We will not go into the different theories in Biology for how this happens. Instead, we noticed that these papers did not always look at how the Nicotine was consumed i.e. through vaping, patches, injection, etc.
...
$a \sim rNorm(100, 25)$ [$a$ Intercept]

$Candidate \sim [1,2,3]$ [$a$ Predictor variables]

$b \sim rNorm(0,1)$ [$b$ Regression coefficient]
...
The mean and SD for each candidate are: Trump = (-2, 3), Bernie = (2, 2) and Hillary = ()
...
We can see that our analysis is able to reproduce the means and standard deviations of our synthetic dataset for each candidate. We conducted a Linear Model analysis to test the sentiment score. The results found a significant effect(F(2, 297) = 39.33, p < 0.001) for the candidates on the sentiment score. The model with the candidates also has the best goodness-of-fit as it has the smaller AICc value.
...
compare(m0F,m1F) #WAIC - Model4 is best out-of-sample fit because of the smallest WAIC value
...
compare(m0,m1) #WAIC - Model4 is best out-of-sample fit because of the smallest WAIC value
...
The credibility interval range of the coefficients for the model with
the real data is much smaller indicating a much more precise range. The
effect of the coefficients stays the same since a in both real and fake
models has a positive effect and b has a negative effect.
...
```{r}
knitr::include_graphics("C:\\GAMES_SETUP\\Q4\\Seminar Research Methodology for Data Science\\Assignments\\A\\assignA\\conceptualmodelQ2.png")
```
...
$pages$ [Response variable]

$\mu = a+b*version+c*portal+d*version*portal$ [linear model]

$a \sim Norm(100, 20)$ [$a$ Intercept]

$version, portal \sim [0,1]$ [$a$ Predictor variables]

$b \sim 2$ [$b$ Regression coefficient]

$c \sim 3$ [$c$ Regression coefficient]

$d \sim 1$ [$d$ Regression coefficient]
...
linear_model_equ = 2+3*versionF+5*portalF+versionF*portalF
...
The value of the coefficients are b=3,c=5,d=1. The same linear equation
will be used for the frequentist and bayesian approach.
...
```{r}
#knitr::include_graphics("C:\\GAMES_SETUP\\Q4\\Seminar Research Methodology for Data Science\\Assignments\\A\\assignA\\EXPERIMENTdesignQ2.png")
```

The experiment design consisted of 4 conditions since we have 2 IVs with
2 values each. Participants would be allocated to any of them through
random assignment.
...
lowest in comparison to the other models suggesting that the effect of
the IVs is not that high in model4 compared to the other models.
...
and their interaction) are highly significant as their p-values are less
than 2.2e-16. This suggests that both the version and portal factors, as
well as their interaction, have a significant impact on the variable
"pagesF". Also the coefficients of the linear model are being
reproduced.
...
We can observe that all the factors (versionNominal, portalNominal, and
their interaction) are highly significant as their p-values are less
than 2.2e-16. This suggests that both the version and portal factors, as
well as their interaction, have a significant impact on the variable
"pages".
...
The summary of all the models from both distributions reveal the lowest
Max Deviance Residuals for model4 and modelP4 i.e. the models with the
interaction effect. These models are significant, have the lowest AICc
value indicating the best fit, hence we choose them for further
...
plot(density(residual_gaussian), main = "Density Plot of Residuals",
     xlab = "Residuals", ylab = "Density")
plot(density(residual_poisson), main = "Density Plot of Residuals",
     xlab = "Residuals", ylab = "Density")

# q-q plot
plot(model4) #gaussian is normally distributed and appears to support normality
plot(modelP4) #poisson appears less normally distributed than the gaussian
```
...
The Q-Q plot of the gaussian distribution appears to be more normally
distributed and seems to support normality. This can perhaps also be
verified by the histogram plot of the residuals.

The Q-Q plot of the poisson distribution appears less normally
distributed than the gaussian distribution.

```{r}
#Distribution assumptions
#a. Autocorrelation
durbinWatsonTest(model4)
durbinWatsonTest(modelP4)
```

We want to examine the underlying assumptions of whether the errors are
independent. There should be no correlation between the residuals. There
is no strong evidence of autocorrelation at lag 1 in the variable being
analyzed for both the models.
...
For model4 all three predictor variables (versionNominal, portalNominal,
and versionNominal:portalNominal) have relatively high tolerance values
above 0.3. This indicates that there is no severe multicollinearity
issue among these predictor variables.

For modelP4 the interaction term has low tolerance of 0.1645658
indicating a relatively higher degree of correlation with other
predictor variables but it does not appear to be severe.
...
that New version of the website has a high effect towards increasing the
number of pages visited for both markets. This is confirmed by fitting a
linear model with the interaction effects which shows that it has the
best fit (lowest AICc and significant p-value). So there is a
...
levels(df$website_version_effect) #to see the level in the new factor

contrastOld <-c(1,-1,0,0) #Only the old website version 
contrastNew <-c(0,0,1,-1) #Only the new website version 

OldEff <- cbind(contrastOld,contrastNew)
contrasts(df$website_version_effect) <- OldEff #now we link the two contrasts with the factor simple

oldEffectModel <-lm(pages ~ website_version_effect , data = df, na.action = na.exclude)
summary.lm(oldEffectModel)
```
...
predictor variables: versionNominal (χ² = 4000.1, df = 1, p \< 2.2e-16),
portalNominal (χ² = 359.7, df = 1, p \< 2.2e-16), and the interaction
term versionNominal:portalNominal (χ² = 870.0, df = 1, p \< 2.2e-16)." A
simple two-way interaction analysis revealed significant effects for all
predictor variables: website_version_effectcontrastOld (t = 12.79, p \<
2.2e-16), website_version_effectcontrastNew (t = -39.52, p \< 2.2e-16),
and the interaction term website_version_effect (t = 65.90, p \<
2.2e-16)."
...
linear_model_equ = 2+3*versionFBayes+5*portalFBayes+versionFBayes*portalFBayes
...
compare(m3,m4)
compare(m0,m1,m2,m3,m4) #WAIC - Model4 is best out-of-sample fit because of the smallest WAIC value
...
credibility interval shows that the coefficient b can be null but
coefficient c and d are non-null.
...
precis(m3PFake, prob=0.95) #95%  credibility interval of coefficients
```

Model m3PFake has the best fit because of the smallest WAIC value. This
is different than the result for gaussian distribution where model m4
i.e. with interaction effect had the best fit. It could be possible that
more number of iterations and samples are needed to achieve the same
results here as well but for computation reasons we leave it as it is.
The credibility interval for m3PFake shows that the coefficients a,b and
c are non null.
...
where model 3 was better but on the real data, the interaction effect
between the IVs appears to fit better. The credibility interval shows
that all coefficients a,b,c and d are non null.
...
χ²(1) = 3921.2, df = 1, p \< 0.001. In addition, We found that the interclass
